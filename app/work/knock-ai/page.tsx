  ...
  return (
    <div className="border border-charcoal/10 rounded-sm overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between px-6 py-5 text-left hover:bg-ivory transition-colors duration-200 group"
      >
        <div className="flex items-baseline gap-4">
          <span className="font-mono text-[10px] text-gold tracking-widest">
            {number}
          </span>
          <span className="font-cormorant text-xl font-semibold text-charcoal group-hover:text-gold transition-colors duration-300">
            {title}
          </span>
        </div>
        <motion.span
          ...
          className="text-gold text-lg font-light flex-shrink-0 ml-4"
        >
          +
        </motion.span>
      </button>
...
function useCountUp(target: number, isInView: boolean, duration = 900) {
...
function MetricCardShell({
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white rounded-[14px] px-8 py-10 text-center overflow-hidden border border-[#E5E7EB] transition-all duration-300 hover:-translate-y-1"
    ...
    >
      {/* Top accent line */}
      <div
        className="absolute top-0 left-0 right-0 h-[3px] transition-opacity duration-300 opacity-60 group-hover:opacity-100"
        style={{ backgroundColor: accentColor }}
      />
      <div className="mb-4">{children}</div>
      <p className="font-mono text-[10px] text-charcoal/40 tracking-[0.08em] uppercase leading-snug">
        {label}
      </p>
    </motion.div>
  );
...
function CountUpCard({ delay = 0 }: { delay?: number }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-50px" });
  const count = useCountUp(1, isInView, 800);
  const [showArrow, setShowArrow] = useState(false);

  useEffect(() => {
    if (count === 1) {
      const t = setTimeout(() => setShowArrow(true), 200);
  ...
  return (
    <div ref={ref}>
      <MetricCardShell label="Full product built from scratch" delay={delay}>
        <p className="font-cormorant text-5xl md:text-6xl font-semibold text-charcoal">
          {count}
          <motion.span
            initial={{ opacity: 0, x: -8 }}
            animate={showArrow ? { opacity: 1, x: 0 } : {}}
...
function LiveCard({ delay = 0 }: { delay?: number }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-50px" });
  ...
  return (
    <div ref={ref}>
      <MetricCardShell label="Features shipped to production" delay={delay} accentColor="#22C55E">
        <div className="flex items-center justify-center gap-3">
          <p className="font-cormorant text-5xl md:text-6xl font-semibold text-charcoal">
            {word.slice(0, visibleChars)}
            <motion.span
              animate={{ opacity: [1, 0] }}
              transition={{ duration: 0.6, repeat: visibleChars < word.length ? Infinity : 0 }}
              className="inline-block w-[2px] h-[0.8em] bg-charcoal/40 align-middle ml-0.5"
              style={{ display: visibleChars < word.length ? "inline-block" : "none" }}
            />
          </p>
          <motion.span
            initial={{ scale: 0 }}
            animate={visibleChars >= word.length ? { scale: 1 } : {}}
            transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}
            className="w-3 h-3 rounded-full bg-green-500 flex-shrink-0 mt-1"
          >
            <motion.span
              animate={{ scale: [1, 1.8, 1], opacity: [0.6, 0, 0.6] }}
              transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
              className="block w-full h-full rounded-full bg-green-500"
            />
          </motion.span>
        </div>
      </MetricCardShell>
...
function ClientCountCard({ delay = 0 }: { delay?: number }) {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-50px" });
  const count = useCountUp(3, isInView, 1000);

  return (
    <div ref={ref}>
      <MetricCardShell label="New clients onboarded post-launch" delay={delay}>
        <p className="font-cormorant text-5xl md:text-6xl font-semibold text-charcoal">
          {count}
        </p>
      </MetricCardShell>
...
function DoodleDrawLine({ className = "" }: { className?: string }) {
  return (
    <motion.svg
      className={`hidden lg:block pointer-events-none absolute ${className}`}
      width="80"
    ...
    >
      <motion.path
        d="M4 16 C12 4, 28 4, 36 12 C44 20, 60 4, 76 10"
        stroke={doodleColor}
        strokeWidth="1.3"
        strokeLinecap="round"
        fill="none"
...
// ── Doodle Arrow (Before → After) ───────────────────────────────────────────
function DoodleArrowDown({ className = "" }: { className?: string }) {
  return (
    <svg
    ...
    >
      <path
        d="M24 4 C20 16, 28 20, 22 32 C16 44, 30 48, 24 56"
        stroke={doodleColor}
        strokeWidth="1.8"
        strokeLinecap="round"
        fill="none"
      ...
      <path
        ...
        fill="none"
      />
      <circle cx="14" cy="18" r="1.5" fill={doodleColor} opacity="0.5" />
      <circle cx="34" cy="40" r="1" fill={doodleColor} opacity="0.4" />
    </svg>
  );
}
